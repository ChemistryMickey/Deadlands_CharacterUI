...
def generate_RNG_tab( RNGTab ):
	...
	Label( RNGTab, text = 'Max of Dice Rolls: ' ).grid( row = 1, column = 0, padx = 10, pady = 10 );
	maxStr = StringVar();
	maxWell = Label( RNGTab, textvariable = maxStr, relief = RAISED, width = 15 ).grid( row = 1, column = 1, padx = 10, pady = 10 );
	...
	Label( RNGTab, text = "Number of Dice to Roll: " ).grid( row = 2, column = 0, padx = 10, pady = 10 );
	drop.grid(row = 2, column = 1, padx = 10, pady = 10);
	db_log( 'Dice dropdown created' );

	# ~ numDice = OptionMenu( RNGTab, clicked, *optionList )
	d4Button = Button( RNGTab, text = 'd4', command = lambda: display_dice( roll_dice( 4, int(clicked.get()), int(enableExplDice.get()) ), rollWellStr, sumStr, maxStr ) );
	d6Button = Button( RNGTab, text = 'd6', command = lambda: display_dice( roll_dice( 6, int(clicked.get()), int(enableExplDice.get()) ), rollWellStr, sumStr, maxStr ) );
	d8Button = Button( RNGTab, text = 'd8', command = lambda: display_dice( roll_dice( 8, int(clicked.get()), int(enableExplDice.get()) ), rollWellStr, sumStr, maxStr ) );
	d10Button = Button( RNGTab, text = 'd10', command = lambda: display_dice( roll_dice( 10, int(clicked.get()), int(enableExplDice.get()) ), rollWellStr, sumStr, maxStr ) );
	d12Button = Button( RNGTab, text = 'd12', command = lambda: display_dice( roll_dice( 12, int(clicked.get()), int(enableExplDice.get()) ), rollWellStr, sumStr, maxStr ) );
	d20Button = Button( RNGTab, text = 'd20', command = lambda: display_dice( roll_dice( 20, int(clicked.get()), int(enableExplDice.get()) ), rollWellStr, sumStr, maxStr ) );
	d100Button = Button( RNGTab, text = 'd100', command = lambda: display_dice( roll_dice( 100, int(clicked.get()), int(enableExplDice.get()) ), rollWellStr, sumStr, maxStr ) );

	d4Button.grid( row = 3, column = 1, padx = 5, pady = 5 );
	d6Button.grid( row = 3, column = 2, padx = 5, pady = 5 );
	d8Button.grid( row = 3, column = 3, padx = 5, pady = 5 );
	d10Button.grid( row = 4, column = 1, padx = 5, pady = 5 );
	d12Button.grid( row = 4, column = 2, padx = 5, pady = 5 );
	d20Button.grid( row = 4, column = 3, padx = 5, pady = 5 );
	d100Button.grid( row = 2, column = 3, padx = 5, pady = 5 );
	db_log( 'Dice Created' );
	...
	drawCardButton.grid( row = 8, column = 0, columnspan = 5, padx = 10, pady = 10 );
